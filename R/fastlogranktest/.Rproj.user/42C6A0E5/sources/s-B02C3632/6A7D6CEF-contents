...
// [[Rcpp::export]]
double logrank_instance(std::vector<double>& groupa, std::vector<double>& groupb, std::vector<int>& groupacensored, std::vector<int>& groupbcensored, bool teststatistic){
...
void startthread(unsigned long long start, unsigned long long end, bool teststatistic) {
  for (unsigned long long j = start; j < end; ++j) {
    pvalues[j]=logrank_instance(groupas[j], groupbs[j], groupacensoreds[j], groupbcensoreds[j], teststatistic);
  }
}

// [[Rcpp::export]]
std::vector<double> cpu_parallel_logrank1(std::vector<std::vector<double>>& groupas_, std::vector<std::vector<double>>& groupbs_, std::vector<std::vector<int>>& groupacensoreds_, std::vector<std::vector<int>> &groupbcensoreds_, unsigned threadnumber, bool teststatistic) {
...
// [[Rcpp::export]]
std::vector<double> cpu_parallel_logrank(std::vector<std::vector<double>>& groupas, std::vector<std::vector<double>>& groupbs, std::vector<std::vector<int>>& groupacensoreds, std::vector<std::vector<int>>& groupbcensoreds, bool teststatistic) {
  return cpu_parallel_logrank1(groupas, groupbs, groupacensoreds, groupbcensoreds, std::thread::hardware_concurrency(), teststatistic);
}
